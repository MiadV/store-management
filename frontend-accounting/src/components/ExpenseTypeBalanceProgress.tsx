...
const ExpenseTypeBalanceProgress: React.FC<{
  ...
  const { data: expenseBalance, isLoading } = useExpenseBalance(expenseRuleId, {
  ...
  useEffect(() => {
    if (expenseBalance) {
      const limit = expenseBalance.limit ? parseFloat(expenseBalance.limit) : null;
  ...
  return (
    <Skeleton isLoaded={!isLoading}>
      {state.limit === null ? (
        <Text fontSize="sm">{`Spend: ${currencyFormat(state.currentTotal)}`}</Text>
      ) : state.currentTotal > state.limit ? (
        <Box>
          <Text fontSize="sm" color="red.500">
            {`${currencyFormat(state.currentTotal - state.limit)} passed ${currencyFormat(
              state.limit
            )} limit`}
          </Text>
        </Box>
        ...
        <Box>
          <Progress variant="outline" value={(state.currentTotal / state.limit) * 100} />
          <Text fontSize="sm">
            {`${currencyFormat(state.currentTotal)} / ${currencyFormat(state.limit)} ${
